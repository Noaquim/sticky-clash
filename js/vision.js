...
export class Vision {
  ...
  isSkin(i, wr, wg, wb) {
    ...
    // Alleen een lichtzweem wegwerken, en begrensd: rood mag omlaag en blauw omhoog,
    // niet andersom. Op een koele muur maakte volledig wegdelen een wit kaartje of
    // gele post-it juist oranje (en dus 'huid'); op kurk, hout of karton — zelf
    // huidkleurig — viel een echte hand weg. Een duidelijk gekleurde muur krijgt
    // daarom helemaal geen correctie.
    if (wr > 0 && wg > 0 && wb > 0) {
      const wmax = Math.max(wr, wg, wb), wmin = Math.min(wr, wg, wb);
      if ((wmax - wmin) / wmax <= 0.35) {
        const m = (wr + wg + wb) / 3;
        r *= Math.min(1, Math.max(0.75, m / wr));
        g *= Math.min(1.1, Math.max(0.9, m / wg));
        b *= Math.min(1.33, Math.max(1, m / wb));
      }
    }
  ...
  findResident(inside) {
    ...
    for (let cy = 0; cy < rows; cy++) {
      ...
      for (let cx = 0; cx < cols; cx++) {
        ...
        const thr = this.objThresh * Math.max(0.3, Math.min(1, wl / 140)) + this.noise[i] * 2.2;
   ...
   */
  setProjection(luma, lw, lh, toUV) {
    const p = this.projLuma;
    if (!luma || !toUV) { p.fill(0); this.projPrev.fill(0); return; }
    ...
    for (let cy = 0; cy < rows; cy++) {
      for (let cx = 0; cx < cols; cx++) {
        ...
        const u = uv[0], v = uv[1];
        if (u < 0 || v < 0 || u > 1 || v > 1) { p[idx] = 0; continue; }
  ...
  classifyObject() {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const br = bg[i * 3] * gain, bg2 = bg[i * 3 + 1] * gain, bb = bg[i * 3 + 2] * gain;
      ...
      const thr = base + pj * 0.85;
      const diff = (Math.abs(r - br) + Math.abs(g - bg2) + Math.abs(b - bb)) / 3;
      ...
      if (pj < 6) this.softW[i] = Math.max(0, Math.min(1, (s - 0.35) / 0.65));
      ...
      const seed = s > 1;
      let on = seed || (prev[i] && s > 0.7);
      if (!on && prev[i] && diff > base && keepAge[i] < 90) {
  ...
  growMarked() {
    ...
    const q = this.stack, cols = this.cols, n = mask.length;
    ...
    if (L) {
      ...
      for (let i = 0; i < n; i++) if (grown[i] === 2) { mask[i] = 0; lab[i] = 0; }
    }
  }
  ...
  blobs() {
    ...
    for (let start = 0; start < mask.length; start++) {
      ...
      out.push({
        cls: team, mx, my, sup, hull, ang, ecc: axis[1], cells: n, touchEdge,
